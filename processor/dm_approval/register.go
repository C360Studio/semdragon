...
// Factory creates a new DM Approval component from configuration.
func Factory(rawConfig json.RawMessage, deps component.Dependencies) (component.Discoverable, error) {
	config := DefaultConfig()

	if len(rawConfig) > 0 {
		if err := json.Unmarshal(rawConfig, &config); err != nil {
			return nil, errs.Wrap(err, "dm_approval", "Factory", "unmarshal config")
		}
	}
...
// Register adds the DM Approval component to the registry.
