...
// fall back to FormatMarkdown so a typo doesn't surprise the caller with a
// hard error and so existing call sites that expect "fetch and read full
// content" continue to work without an explicit format argument.
