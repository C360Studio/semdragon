...
func TestToolNameAllowed(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			allowed: []string{"bash", "http_request", "search"},
			tool:    "http_request",
			want:    true,
		},
		{
			name:    "tool is first in list",
			allowed: []string{"bash", "http_request"},
			tool:    "bash",
			want:    true,
		},
		{
...
func TestToolChoiceForQuest(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			quest:    &domain.Quest{PartyRequired: true},
			agent:    &agentprogression.Agent{Tier: domain.TierJourneyman},
			tools:    makeTools("bash", "submit_work"),
			wantMode: "required",
		},
		{
			...
			quest:        &domain.Quest{},
			agent:        &agentprogression.Agent{Tier: domain.TierJourneyman},
			tools:        makeTools("submit_work"),
			wantMode:     "function",
			wantFunction: "submit_work",
		},
		{
			name:     "multiple tools uses required",
			quest:    &domain.Quest{},
			agent:    &agentprogression.Agent{Tier: domain.TierExpert},
			tools:    makeTools("bash", "http_request", "submit_work"),
			wantMode: "required",
		},
		{
			...
			agent:    &agentprogression.Agent{Tier: domain.TierJourneyman},
			tools:    makeTools("bash", "ask_clarification", "submit_work"),
			wantMode: "required",
		},
	}
