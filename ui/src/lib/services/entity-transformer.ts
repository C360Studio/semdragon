...
function transformBattle(key: string, entity: GraphEntity): BossBattle {
	...
	return {
		id: battleId(key),
		quest_id: questId(str(m.get('battle.quest.id'))),
		agent_id: agentId(str(m.get('battle.agent.id'))),
...
function transformParty(key: string, entity: GraphEntity): Party {
	...
	return {
		...
		quest_id: questId(str(m.get('party.quest.id'))),
		lead: agentId(str(m.get('party.lead.agent'))),
		members: [],
		strategy: str(m.get('party.strategy')),
		sub_quest_map: {},
		shared_context: [],
		sub_results: {},
