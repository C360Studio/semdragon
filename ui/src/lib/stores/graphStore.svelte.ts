/**
 * Game Entity Graph Store
 *
 * Manages state for the graph visualization tab in the Semdragons dashboard.
 * Handles game entities, relationships, selection state, and type filters.
 *
 ...
 *
 * Ported from semstreams-ui graphStore and adapted for semdragons:
 * - Default prefix filters to game entities (.game. in entity ID)
 * - Default limit of 200 nodes
 * - Entity type filter for quest/agent/party/guild/battle/peer_review
 * - Imports from $lib/services/graphApi (to be created separately)
 */
...
// shape — see the graphApiAdapter in routes/graph/+page.svelte.
...
function createGraphStore() {
	...
	/** Unique game entity types present in the current data set. */
	...
	return {
		 ...
		 *
		 * @param api      - The graphApi service instance
		 * @param prefix   - Entity ID prefix filter; defaults to game entities (.game.)
		 * @param limit    - Max nodes to load; defaults to 200
		 */
		async loadInitialGraph(
			api: GraphStoreAdapter,
			prefix: string = DEFAULT_GAME_PREFIX,
		 ...
		 *
		 * @param api      - The graphApi service instance
		 * @param entityId - The full 6-part entity ID to expand
		 */
		 ...
		 */
		async search(api: GraphStoreAdapter, query: string): Promise<void> {
