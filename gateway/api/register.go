...
// Factory creates a new API Gateway component from configuration.
func Factory(rawConfig json.RawMessage, deps component.Dependencies) (component.Discoverable, error) {
	...
	if len(rawConfig) > 0 {
		if err := json.Unmarshal(rawConfig, &config); err != nil {
			return nil, errs.Wrap(err, "api", "Factory", "unmarshal config")
		}
	}
...
// Register adds the API Gateway component to the registry.
func Register(registry *component.Registry) error {
	return registry.RegisterWithConfig(component.RegistrationConfig{
		...
		Protocol:    "http",
		Domain:      "api",
		Description: "HTTP API gateway for semdragons",
		Version:     "1.0.0",
	})
}
